...
def load_historical_dividend(
    repo: HistoricalDividendAbstractRepository, ds: Datasource, symbol: str
):
...
def load_delisted_companies(
    repo: DelistedCompanyAbstractRepository, ds: Datasource, page: int = 0
):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Load data from financialmodelingprep.com into a database",
        prog="fmp_loader",
    )
    ...
    sg.add_argument(
        "--api",
        help="Load data from FMP API, an FMP_API_KEY is required in .env file",
        action="store_true",
    )
    ...
    dc_sp.add_argument(
        ...
        type=int,
        nargs="?",
        help="load delisted companies data for the given page, if not specified, default is 0 (doesn't work with local file)",
        default=0,
    )
